...
import time
from src.utils.metrics import Tracker, calculate_rmse, calculate_omat, is_positive_definite, condition_number,analyze_stability
...
class TestMetrics(unittest.TestCase):
    ...
    def test_analyze_stability(self):
        """Test symmetry error of covariance matrices."""
        # Perfectly symmetric matrices → error = 0
        sym_matrices = np.array([
            [[1.0, 0.5], [0.5, 2.0]],
            [[3.0, 0.0], [0.0, 4.0]]
        ])
        mean_err, max_err = analyze_stability(sym_matrices)
        self.assertAlmostEqual(mean_err, 0.0)
        self.assertAlmostEqual(max_err, 0.0)

        # Asymmetric matrix (upper triangular)
        asym_mat = np.array([
            [[1.0, 1.0], [0.0, 1.0]]   # diff = [[0,1],[-1,0]] → Frobenius norm = √2 ≈1.414
        ])
        mean_err, max_err = analyze_stability(asym_mat)
        expected = np.sqrt(2.0)   # ≈ 1.41421356
        self.assertAlmostEqual(mean_err, expected, places=5)
        self.assertAlmostEqual(max_err, expected, places=5)

        # Single matrix provided (shape (1,2,2))
        single_mat = np.array([[[2.0, 0.0], [1.0, 2.0]]])
        mean_err, max_err = analyze_stability(single_mat)
        # diff = [[0, -1], [1, 0]] → Frobenius norm = √2
        self.assertAlmostEqual(mean_err, expected, places=5)
        self.assertAlmostEqual(max_err, expected, places=5)
