...
class TestKalmanFilter(unittest.TestCase):
    ...
    def test_run_filter(self):
        """
        verify both standard and joseph modes work correctly and produce valid output with correct shapes.
        """
        ...
        self.assertEqual(means_std.shape, (self.num_timesteps, self.state_dim))
        self.assertEqual(covs_std.shape, (self.num_timesteps, self.state_dim, self.state_dim))
        # Assert no NaN/Inf values in the results
        self.assertFalse(tf.reduce_any(tf.math.is_nan(means_std)))
        self.assertFalse(tf.reduce_any(tf.math.is_nan(covs_std)))

        # 2. Test joseph mode
        self.kf.initialize(self.initial_mean, self.initial_cov)
        means_joseph, covs_joseph = self.kf.run_filter(observations, method='joseph')
        # Assert output shapes are correct
        self.assertEqual(means_joseph.shape, (self.num_timesteps, self.state_dim))
        self.assertEqual(covs_joseph.shape, (self.num_timesteps, self.state_dim, self.state_dim))

        # 3. Assert results from both modes are consistent
        np.testing.assert_allclose(means_std.numpy(), means_joseph.numpy(), rtol=1e-5, atol=1e-6)
        np.testing.assert_allclose(covs_std.numpy(), covs_joseph.numpy(), rtol=1e-5, atol=1e-6)
