...
class TestPlot(unittest.TestCase):
    ...
    @patch('matplotlib.pyplot.show')
    def test_plot_homotopy_results_with_data(self, mock_show):
        lambdas = np.linspace(0, 1, 10)
        beta_opt = lambdas ** 2
        beta_dot_opt = 2 * lambdas
        R_stiff_opt = np.exp(lambdas)
        R_stiff_base = np.exp(0.5 * lambdas)

        # Should not raise any exception
        plot_homotopy_results(lambdas, beta_opt, beta_dot_opt, R_stiff_opt, R_stiff_base)

        # Check that plt.show was called
        mock_show.assert_called_once()

    @patch('matplotlib.pyplot.show')
    def test_plot_homotopy_results_without_R_stiff(self, mock_show):
        lambdas = np.linspace(0, 1, 10)
        beta_opt = lambdas ** 2
        beta_dot_opt = 2 * lambdas

        plot_homotopy_results(lambdas, beta_opt, beta_dot_opt)

        # Still should call plt.show
        mock_show.assert_called_once()

    @patch('matplotlib.pyplot.show')
    def test_plot_homotopy_results_with_partial_R_stiff(self, mock_show):
        lambdas = np.linspace(0, 1, 10)
        beta_opt = lambdas ** 2
        beta_dot_opt = 2 * lambdas
        R_stiff_opt = np.exp(lambdas)
        # R_stiff_base is None
        plot_homotopy_results(lambdas, beta_opt, beta_dot_opt, R_stiff_opt=R_stiff_opt, R_stiff_base=None)
