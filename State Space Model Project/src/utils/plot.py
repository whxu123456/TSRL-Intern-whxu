...
def plot_acoustic_tracking(sensors_pos, x_true, est_traj_dict, n_targets, filename=None):
    ...
    for i in range(n_targets):
        ...
        ax.plot(tx[0], ty[0], marker='x', color=colors[i % len(colors)], markersize=10)  # start point
...
def plot_homotopy_results(lambdas, beta_opt, beta_dot_opt, R_stiff_opt=None, R_stiff_base=None):
    fig, axs = plt.subplots(2, 2, figsize=(12, 10))

    # (a) beta(lambda)
    axs[0, 0].plot(lambdas, beta_opt, 'r-', linewidth=1.5, label=r'optimal $\beta^*(\lambda)$')
    axs[0, 0].plot(lambdas, lambdas, 'b--', linewidth=1.5, label=r'$\beta(\lambda) = \lambda$')
    axs[0, 0].set_xlabel(r'$\lambda$')
    axs[0, 0].set_ylabel(r'$\beta(\lambda)$')
    axs[0, 0].set_title('(a)', y=-0.2)
    axs[0, 0].legend()
    axs[0, 0].grid(True, linestyle=':', alpha=0.6)

    # (b) e = beta*(lambda) - lambda
    axs[0, 1].plot(lambdas, beta_opt - lambdas, 'b-', linewidth=1.5)
    axs[0, 1].set_xlabel(r'$\lambda$')
    axs[0, 1].set_ylabel(r'$e = \beta^*(\lambda) - \lambda$')
    axs[0, 1].set_title('(b)', y=-0.2)
    axs[0, 1].grid(True, linestyle=':', alpha=0.6)

    # (c) beta_dot
    axs[1, 0].plot(lambdas, beta_dot_opt, 'b-', linewidth=1.5)
    axs[1, 0].set_xlabel(r'$\lambda$')
    axs[1, 0].set_ylabel(r'$u^*(\lambda)$')
    axs[1, 0].set_title('(c)', y=-0.2)
    axs[1, 0].grid(True, linestyle=':', alpha=0.6)

    # (d) R_stiff
    if R_stiff_opt is not None:
        axs[1, 1].plot(lambdas, R_stiff_opt, 'r-', linewidth=1.5, label=r'optimal $\beta^*(\lambda)$')

        if R_stiff_base is not None:
            axs[1, 1].plot(lambdas, R_stiff_base, 'b--', linewidth=1.5, label=r'$\beta(\lambda) = \lambda$')

        axs[1, 1].set_yscale('log')

        axs[1, 1].set_xlabel(r'$\lambda$')
        axs[1, 1].set_ylabel(r'$R_{stiff}$')
        axs[1, 1].set_title('(d)', y=-0.2)
        axs[1, 1].legend()
        axs[1, 1].grid(True, linestyle=':', alpha=0.6)
    else:
        axs[1, 1].text(0.5, 0.5, 'R_stiff data not provided',
